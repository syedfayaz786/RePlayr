      ...
      <div className="p-2 sm:p-3">
        ...
        <div className="flex items-center gap-1.5 mb-1.5 min-w-0">
          {listing.seller.image ? (
            <img src={listing.seller.image} alt="" className="w-4 h-4 rounded-full object-cover flex-shrink-0" />
          ) : (
            <div className="w-4 h-4 rounded-full bg-brand-500/30 flex items-center justify-center text-brand-300 text-[9px] font-bold flex-shrink-0">
              {listing.seller.name?.[0]?.toUpperCase() ?? "?"}
            </div>
          )}
          <span className="text-xs text-gray-400 truncate">{listing.seller.name ?? "Seller"}</span>
          {listing.seller.avgRating !== undefined && listing.seller.reviewCount ? (
            <span className="flex items-center gap-0.5 flex-shrink-0 ml-auto">
              ...
              <span className="text-xs text-gray-600">({listing.seller.reviewCount})</span>
            </span>
          ) : null}
        </div>
